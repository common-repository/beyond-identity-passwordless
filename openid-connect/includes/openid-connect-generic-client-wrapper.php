...
class BYNDID_OpenID_Connect_Generic_Client_Wrapper
{
	 ...
	 * @var string
	 */
	private $cookie_token_refresh_key = 'beyond-identity-passwordless-refresh';
	 ...
	 *
	 * @param BYNDID_OpenID_Connect_Generic_Client          $client   A plugin client object instance.
	 * @param BYNDID_OpenID_Connect_Generic_Option_Settings $settings A plugin settings object instance.
	 */
	public function __construct(BYNDID_OpenID_Connect_Generic_Client $client, BYNDID_OpenID_Connect_Generic_Option_Settings $settings)
	{
		$this->client = $client;
	 ...
	 *
	 * @param \BYNDID_OpenID_Connect_Generic_Client          $client   The plugin client instance.
	 * @param \BYNDID_OpenID_Connect_Generic_Option_Settings $settings The plugin settings instance.
	 *
	 * @return \BYNDID_OpenID_Connect_Generic_Client_Wrapper
	 */
	public static function register(BYNDID_OpenID_Connect_Generic_Client $client, BYNDID_OpenID_Connect_Generic_Option_Settings $settings)
	{
		...
		add_filter('beyond-identity-passwordless-alter-request', array($client_wrapper, 'alter_request'), 10, 3);
	...
	public function ensure_tokens_still_fresh()
	{
		...
		if (!$refresh_token || ($refresh_expires && $current_time > $refresh_expires)) {
			if (isset($_SERVER['REQUEST_URI'])) {
				...
				if ($this->settings->redirect_on_logout) {
					$this->error_redirect(new WP_Error('access-token-expired', esc_html__('Session expired. Please login again.', 'beyond-identity-passwordless')));
				}
		...
		update_user_meta($user_id, 'beyond-identity-passwordless-last-token-response', $token_response);
	...
	public function get_end_session_logout_redirect_url($redirect_url, $requested_redirect_to, $user)
	{
		...
		$token_response = $user->get('beyond-identity-passwordless-last-token-response');
		if (!$token_response) {
		...
		$claim = $user->get('beyond-identity-passwordless-last-id-token-claim');
	...
	public function authentication_request_callback()
	{
		...
		$code = $client->get_authentication_code();
		...
		$state = $client->get_authentication_state();
		...
		$token_response = apply_filters('beyond-identity-passwordless-modify-token-response-before-validation', $token_response);
		...
		// Allow for other plugins to alter data before validation.
		$id_token_claim = apply_filters('beyond-identity-passwordless-modify-id-token-claim-before-validation', $id_token_claim);
		...
		if ($user) {
			add_user_meta($user->ID, 'beyond_identity_user_id', $id_token_claim['sub'], true);
		}
		...
		if (!$user) {
			...
			if ($this->settings->link_existing_users || $this->settings->create_if_does_not_exist) {
				...
				$this->error_redirect(new WP_Error('identity-not-map-existing-user', __('User identity is not linked to an existing WordPress user.', 'beyond-identity-passwordless'), $user_claim));
			}
		}
		...
		$state_object = get_transient('beyond-identity-passwordless-state--' . $state);
	...
	public function validate_user($user)
	{
		...
		if (!is_a($user, 'WP_User') || !$user->exists()) {
			return new WP_Error('invalid-user', __('Invalid user.', 'beyond-identity-passwordless'), $user);
		}
	...
	public function refresh_user_claim($user, $token_response)
	{
		...
		// Allow for other plugins to alter data before validation.
		$id_token_claim = apply_filters('beyond-identity-passwordless-id-token-claim-before-validation', $id_token_claim);
		...
		update_user_meta($user->ID, 'beyond-identity-passwordless-last-token-response', $token_response);
		update_user_meta($user->ID, 'beyond-identity-passwordless-last-id-token-claim', $id_token_claim);
		update_user_meta($user->ID, 'beyond-identity-passwordless-last-user-claim', $user_claim);
	...
	{
		// Look for user by their beyond-identity-passwordless-subject-identity value.
		$user_query = new WP_User_Query(
			array(
				'meta_query' => array(
					array(
						'key'   => 'beyond-identity-passwordless-subject-identity',
	...
	private function get_username_from_claim($user_claim)
	{
		...
		if (empty($desired_username)) {
			...
			return new WP_Error('no-username', __('No appropriate username found.', 'beyond-identity-passwordless'), $user_claim);
		}
		...
		if (empty($sanitized_username)) {
			...
			return new WP_Error('username-sanitization-failed', sprintf(__('Username %1$s could not be sanitized.', 'beyond-identity-passwordless'), $desired_username), $desired_username);
		}
	...
	private function get_nickname_from_claim($user_claim)
	{
		...
		if (empty($desired_nickname)) {
			...
			return new WP_Error('no-nickname', sprintf(__('No nickname found in user claim using key: %1$s.', 'beyond-identity-passwordless'), $this->settings->nickname_key), $this->settings->nickname_key);
		}
				...
				if (!$this->get_claim($key, $user_claim, $info)) {
					if ($error_on_missing_key) {
						return new WP_Error(
							...
							__('User claim incomplete.', 'beyond-identity-passwordless'),
							array(
	...
	public function create_new_user($subject_identity, $user_claim)
	{
		...
		if ($values_missing && isset($user_claim['access_token']) && !empty($this->settings->endpoint_userinfo)) {
			...
			if (is_wp_error($user_claim_result)) {
				return new WP_Error('bad-user-claim-result', __('Bad user claim result.', 'beyond-identity-passwordless'), $user_claim_result);
			}
		 ...
		 */
		$create_user = apply_filters('beyond-identity-passwordless-user-creation-test', $this->settings->create_if_does_not_exist, $user_claim);

		if (!$create_user) {
			return new WP_Error('cannot-authorize', __('Can not authorize.', 'beyond-identity-passwordless'), $create_user);
		}
		...
		$user_data = apply_filters('beyond-identity-passwordless-alter-user-data', $user_data, $user_claim);
		...
		if (is_wp_error($uid)) {
			return new WP_Error('failed-user-creation', __('Failed user creation.', 'beyond-identity-passwordless'), $uid);
		}
		...
		add_user_meta($user->ID, 'beyond-identity-passwordless-subject-identity', (string) $subject_identity, true);
	...
	public function update_existing_user($uid, $subject_identity)
	{
		...
		update_user_meta($uid, 'beyond-identity-passwordless-subject-identity', strval($subject_identity));
