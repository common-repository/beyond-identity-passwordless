...
class BYNDID_OpenID_Connect_Generic_Client
{

	/**
	 * The OIDC/oAuth client ID.
	 *
	 * @see BYNDID_OpenID_Connect_Generic_Option_Settings::client_id
	 *
	 * @var string
	 */
	private $client_id;
	 ...
	 *
	 * @see BYNDID_OpenID_Connect_Generic_Option_Settings::client_secret
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see BYNDID_OpenID_Connect_Generic_Option_Settings::scope
	 *
	 * @var string
	 */
	private $scope;
	 ...
	 *
	 * @see BYNDID_OpenID_Connect_Generic_Option_Settings::endpoint_login
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see BYNDID_OpenID_Connect_Generic_Option_Settings::endpoint_userinfo
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see BYNDID_OpenID_Connect_Generic_Option_Settings::endpoint_token
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see BYNDID_OpenID_Connect_Generic_Option_Settings::redirect_uri
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see BYNDID_OpenID_Connect_Generic_Option_Settings::acr_values
	 *
	 * @var string
	 */
	private $acr_values;
	 ...
	 *
	 * @see BYNDID_OpenID_Connect_Generic_Option_Settings::state_time_limit
	 *
	 * @var int
	 */
	 ...
	 * Client constructor.
	 *
	 * @param string                               $client_id         @see BYNDID_OpenID_Connect_Generic_Option_Settings::client_id for description.
	 * @param string                               $client_secret     @see BYNDID_OpenID_Connect_Generic_Option_Settings::client_secret for description.
	 * @param string                               $scope             @see BYNDID_OpenID_Connect_Generic_Option_Settings::scope for description.
	 * @param string                               $endpoint_login    @see BYNDID_OpenID_Connect_Generic_Option_Settings::endpoint_login for description.
	 * @param string                               $endpoint_userinfo @see BYNDID_OpenID_Connect_Generic_Option_Settings::endpoint_userinfo for description.
	 * @param string                               $endpoint_token    @see BYNDID_OpenID_Connect_Generic_Option_Settings::endpoint_token for description.
	 * @param string                               $redirect_uri      @see BYNDID_OpenID_Connect_Generic_Option_Settings::redirect_uri for description.
	 * @param string                               $acr_values        @see BYNDID_OpenID_Connect_Generic_Option_Settings::acr_values for description.
	 * @param int                                  $state_time_limit  @see BYNDID_OpenID_Connect_Generic_Option_Settings::state_time_limit for description.
	 */
	public function __construct($client_id, $client_secret, $scope, $endpoint_login, $endpoint_userinfo, $endpoint_token, $redirect_uri, $acr_values, $state_time_limit)
	{
	...
	public function get_authentication_code()
	{
		...
		if (!isset($code)) {
			return new WP_Error('missing-authentication-code', __('Missing authentication code.', 'beyond-identity-passwordless'));
		}
	...
	public function request_authentication_token($code)
	{
		...
		$request = apply_filters('beyond-identity-passwordless-alter-request', $request, 'get-authentication-token');

		// Call the server and ask for a token.
		$response = wp_remote_post($this->endpoint_token, $request);

		if (is_wp_error($response)) {
			$response->add('request_authentication_token', __('Request for authentication token failed.', 'beyond-identity-passwordless'));
		}
	...
	public function request_new_tokens($refresh_token)
	{
		...
		// Allow modifications to the request.
		$request = apply_filters('beyond-identity-passwordless-alter-request', $request, 'refresh-token');

		// Call the server and ask for new tokens.
		$response = wp_remote_post($this->endpoint_token, $request);

		if (is_wp_error($response)) {
			$response->add('refresh_token', __('Refresh token failed.', 'beyond-identity-passwordless'));
		}
	...
	public function get_token_response($token_result)
	{
		if (!isset($token_result['body'])) {
			return new WP_Error('missing-token-body', __('Missing token body.', 'beyond-identity-passwordless'), $token_result);
		}
		...
		if (is_null($token_response)) {
			return new WP_Error('invalid-token', __('Invalid token.', 'beyond-identity-passwordless'), $token_result);
		}
	...
	{
		// Allow modifications to the request.
		$request = apply_filters('beyond-identity-passwordless-alter-request', array(), 'get-userinfo');
		...
		if (is_wp_error($response)) {
			$response->add('request_userinfo', __('Request for userinfo failed.', 'beyond-identity-passwordless'));
		}
	...
	public function new_state($redirect_to)
	{
		...
		$transient = 'beyond-identity-passwordless--' . $state;
		set_transient($transient, $state_value, $this->state_time_limit);
	...
	public function check_state($state)
	{
		...
		if (!get_option('_transient_beyond-identity-passwordless--' . $state)) {
			$state_found = false;
		}

		$valid = get_transient('beyond-identity-passwordless--' . $state);

		return $state_found && boolval($valid);
	}
	...
	public function get_authentication_state()
	{
		...
		if (!$this->check_state($state)) {
			return new WP_Error('invalid-state', __('Invalid state.', 'beyond-identity-passwordless'), $state);
		}

		if (!isset($state)) {
			return new WP_Error('missing-authentication-state', __('Missing authentication state.', 'beyond-identity-passwordless'));
		}
	...
	public function get_id_token_claim($token_response)
	{
		...
		if (!isset($token_response['id_token'])) {
			return new WP_Error('no-identity-token', __('No identity token.', 'beyond-identity-passwordless'), $token_response);
		}
		...
		if (!isset($tmp[1])) {
			return new WP_Error('missing-identity-token', __('Missing identity token.', 'beyond-identity-passwordless'), $token_response);
		}
	...
	public function validate_id_token_claim($id_token_claim)
	{
		if (!is_array($id_token_claim)) {
			return new WP_Error('bad-id-token-claim', __('Bad ID token claim.', 'beyond-identity-passwordless'), $id_token_claim);
		}

		// Validate the identification data and it's value.
		if (!isset($id_token_claim['sub']) || empty($id_token_claim['sub'])) {
			return new WP_Error('no-subject-identity', __('No subject identity.', 'beyond-identity-passwordless'), $id_token_claim);
		}
		...
		if (!empty($this->acr_values) && isset($id_token_claim['acr'])) {
			if ($this->acr_values != $id_token_claim['acr']) {
				return new WP_Error('no-match-acr', __('No matching acr values.', 'beyond-identity-passwordless'), $id_token_claim);
			}
		}
	...
	public function get_user_claim($token_response)
	{
		...
		if (is_wp_error($user_claim_result) || !isset($user_claim_result['body'])) {
			return new WP_Error('bad-claim', __('Bad user claim.', 'beyond-identity-passwordless'), $user_claim_result);
		}
	...
	public function validate_user_claim($user_claim, $id_token_claim)
	{
		...
		if (!is_array($user_claim)) {
			return new WP_Error('invalid-user-claim', __('Invalid user claim.', 'beyond-identity-passwordless'), $user_claim);
		}

		// Allow for errors from the IDP.
		if (isset($user_claim['error'])) {
			$message = __('Error from the IDP.', 'beyond-identity-passwordless');
		...
		if ($id_token_claim['sub'] !== $user_claim['sub']) {
			return new WP_Error('incorrect-user-claim', __('Incorrect user claim.', 'beyond-identity-passwordless'), func_get_args());
		}

		// Allow for other plugins to alter the login success.
		$login_user = apply_filters('beyond-identity-passwordless-user-login-test', true, $user_claim);

		if (!$login_user) {
			return new WP_Error('unauthorized', __('Unauthorized access.', 'beyond-identity-passwordless'), $login_user);
		}
